package com.omniscien.lslanguageid.model;

public class ResponseMessage {
	
	private String duration;
	private String result;
	private String errorno;
	private String dominantlanguage;
	private String enddate;
	private String secondarylanguage;
	private String errortext;
	private String startdate;
	
	
	
	public ResponseMessage() {
		
	}
	
	

	public String getDuration() {
		return duration;
	}



	public void setDuration(String duration) {
		this.duration = duration;
	}



	public String getResult() {
		return result;
	}



	public void setResult(String result) {
		this.result = result;
	}



	public String getErrorno() {
		return errorno;
	}



	public void setErrorno(String errorno) {
		this.errorno = errorno;
	}



	public String getDominantlanguage() {
		return dominantlanguage;
	}



	public void setDominantlanguage(String dominantlanguage) {
		this.dominantlanguage = dominantlanguage;
	}



	public String getEnddate() {
		return enddate;
	}



	public void setEnddate(String enddate) {
		this.enddate = enddate;
	}



	public String getSecondarylanguage() {
		return secondarylanguage;
	}



	public void setSecondarylanguage(String secondarylanguage) {
		this.secondarylanguage = secondarylanguage;
	}



	public String getErrortext() {
		return errortext;
	}



	public void setErrortext(String errortext) {
		this.errortext = errortext;
	}



	public String getStartdate() {
		return startdate;
	}



	public void setStartdate(String startdate) {
		this.startdate = startdate;
	}



	@Override
	public String toString() {
		return "ResponseMessage [duration = "+duration+", result = "+result+", errorno = "+errorno+", dominantlanguage = "+dominantlanguage+", enddate = "+enddate+", secondarylanguage = "+secondarylanguage+", errortext = "+errortext+", startdate = "+startdate+"]";
	}
	
	
	
}
